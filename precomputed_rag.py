...
class EnhancedPrecomputedRAGSystem:
    ...
    def __init__(self, embeddings_dir: Optional[Path] = None):
        """Initialize the enhanced RAG system with lazy model loading"""
        ...
        try:
            # Load precomputed data first (lightweight)
            self._load_precomputed_data()
            
            # Model will be loaded on first search (lazy loading)
            logger.info("RAG system ready (model will load on first use)")
            self.is_ready = True
        ...
        try:
            ...
            if documents_path.exists():
                ...
                if isinstance(data, dict):
                    ...
                    logger.info(f"Loaded {len(self.documents)} enhanced documents (from dict)")
                else:
                    self.documents = data
                    logger.info(f"Loaded {len(self.documents)} enhanced documents (from list)")
...
if __name__ == "__main__":
    ...
    try:
        ...
        test_results = rag.search("HBA", top_k=3)
        if test_results:
            print("✅ RAG search successful! Found:")
            for res in test_results:
                print(f"- {res.content[:50]}... (Source: {res.source})")
        else:
            print("❌ RAG search FAILED: No results returned.")
    except Exception as e:
